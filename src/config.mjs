...
const ED25519_PUBLIC_HEX = /^[0-9a-f]{64}$/;
...
export function runtimeConfig(raw = {}) {
  ...
  const defaults = {
    ...
    requireApproval: [], memoryReadTools: [], memoryWriteTools: [], memoryTaskStartTool: '', memoryPublishTool: '',
    memoryReceiptPublicKey: '', recall: { mode: 'advise', tools: [] }, structuredOperationTools: [], targets: {}
  };
  ...
  for (const key of ['memoryTaskStartTool', 'memoryPublishTool']) check(config[key] === '' || (typeof config[key] === 'string' && NAME.test(config[key])), 'INVALID_TOOL_ALLOWLIST');
  // The start tool is the one write that may name a task key nobody has observed yet.
  check(config.memoryTaskStartTool === '' || config.memoryWriteTools.includes(config.memoryTaskStartTool), 'MEMORY_START_TOOL_NOT_WRITE');
  check(config.memoryPublishTool === '' || !config.memoryWriteTools.includes(config.memoryPublishTool), 'MEMORY_PUBLISH_TOOL_IS_WRITE');
  check(config.memoryReceiptPublicKey === '' || ED25519_PUBLIC_HEX.test(config.memoryReceiptPublicKey), 'INVALID_RECEIPT_PUBLIC_KEY');
